...
SEAWATER_DENSITY_NG  = 1.0255    # t/m³ — Nigerian coastal/Bonny River estuary (locked)
RHO_STABILITY_REF    = 1.025     # t/m³ — IACS/ILLC Cb derivation reference (stability booklet standard)
...
# ── Loading water density presets ────────────────────────────────────────────
# The water at the LOADING POINT (river vs offshore) has a different density
# from the open sea. This governs displacement (and draft) while loading.
# River locations (San Barth, Awoba, Chapel/JasmineS bar): essentially freshwater
# Offshore BIA berths: Nigerian coastal seawater (1.0255 t/m³)
# Using 1.0255 for river loading overestimates displacement by ~3,500+ Bbls.
LOADING_WATER_PRESETS: dict[str, float | None] = {
    "— Manual entry —":                              None,
    "River — San Barth / Awoba / Cawthorne (1.000 t/m³)": 1.000,
    "River — Chapel / JasmineS bar (1.000 t/m³)":       1.000,
    "BIA / Offshore mother vessel berth (1.0255 t/m³)":  1.0255,
    "Lightly brackish creek (1.005 t/m³)":               1.005,
    "Brackish estuary (1.010 t/m³)":                     1.010,
}
...
def _cb_for_stowage(v: dict) -> float:
    """
    Derive the effective block coefficient for stowage displacement calculations.

    Per workbook and IACS convention:
        Cb = (full_displacement / 1.025) / (LOA × BEAM × TLD)

    Stability booklets quote displacement at ρ_ref=1.025 t/m³ (standard seawater).
    This Cb is then applied at the actual loading water density:
        Δ(T) = Cb × LOA × BEAM × T × ρ_loading

    Verified against Bedford workbook (API 28.9°, constant 350, bunker+FW 830.6 MT):
        T=5.000 m  →  67,000 Bbls  ✓
        T=5.085 m  →  69,000 Bbls  ✓
        T=5.175 m  →  71,000 Bbls  ✓
    """
    tld   = v["draft_full"]
    denom = RHO_STABILITY_REF * v["loa"] * v["beam"] * tld
    return v["displacement"] / denom if denom > 0 else 0.75


def _displacement_at_draft(v: dict, draft_m: float, rho_loading: float) -> float:
    """
    Compute displacement at a partial draft using the correct two-density approach.

    Δ(T) = Cb × LOA × BEAM × T × ρ_loading
    where Cb = (full_displacement / 1.025) / (LOA × BEAM × TLD)

    WHY NOT SIMPLE PROPORTION (T/TLD × full_disp)?
    That ignores the density difference between the stability reference (ρ=1.025)
    and the actual loading water (ρ_river ≈ 1.000). The error is:
        error ≈ full_disp × (T/TLD) × (1 − ρ_loading/1.025)
    ≈ 450 MT at T=5m for a GP tanker, equivalent to ~3,200 Bbls overestimation.
    """
    Cb = _cb_for_stowage(v)
    return Cb * v["loa"] * v["beam"] * draft_m * rho_loading


def _draft_from_displacement(v: dict, displacement_mt: float,
                              rho_loading: float) -> float:
    """Inverse of _displacement_at_draft — find draft given total displacement."""
    Cb    = _cb_for_stowage(v)
    denom = Cb * v["loa"] * v["beam"] * rho_loading
    return displacement_mt / denom if denom > 0 else 0.0


def volume_to_draft(v: dict, volume_m3: float, api: float,
                    bunker_mt: float, fw_mt: float, constant_mt: float,
                    rho_loading: float = 1.000) -> dict:
    """
    Predict loaded draft from cargo volume.

    Method (IACS / workbook standard):
      Cb = (full_displacement / 1.025) / (LOA × BEAM × TLD)
      loaded_displacement = lightship + cargo_mass + deductions
      draft = loaded_displacement / (Cb × LOA × BEAM × rho_loading)

    rho_loading is the density of the LOADING WATER (not seawater):
      River locations (San Barth, Awoba, Chapel/JasmineS bar): 1.000 t/m³
      Offshore/BIA berth: 1.0255 t/m³
    ...
    # Draft using Cb-based formula with loading water density
    draft_m = _draft_from_displacement(v, total_disp, rho_loading)
...
def draft_to_volume(v: dict, draft_m: float, api: float,
    ...
    Method (IACS / workbook standard):
      Cb = (full_displacement / 1.025) / (LOA × BEAM × TLD)
      displacement_at_draft = Cb × LOA × BEAM × draft × rho_loading
      cargo_mass = displacement_at_draft - lightship - deductions
      volume = cargo_mass / SG

    rho_loading: density of water at the loading point.
    Clamps result to binding limit (98% tank or Tropical DWT, whichever is smaller).
    """
    sg         = api_to_sg(api)
    limits     = compute_limits(v, bunker_mt, fw_mt, constant_mt, api)
    lightship  = lightship_mass(v)
    ...
    # Displacement at this draft using Cb and loading water density
    total_disp     = _displacement_at_draft(v, draft_m, rho_loading)
...
with st.sidebar:
    ...
    st.markdown("---")
    locked_display("Offshore Seawater ρ (Nigerian Waters)",
                   f"{SEAWATER_DENSITY_NG} t/m\u00b3")
    st.markdown(
        "<div style='font-size:0.68rem;color:var(--muted);font-family:var(--mono);"
        "margin-top:2px;margin-bottom:6px'>"
        "Bonny coastal · 25–26 ppt · 28°C · locked (UKC reference)"
        "</div>", unsafe_allow_html=True)
    ...
    # ── Loading water density ─────────────────────────────────────────────
    st.markdown("---")
    st.markdown("<span style='font-size:0.75rem;color:var(--muted);font-family:var(--mono)'>LOADING WATER DENSITY</span>",
                unsafe_allow_html=True)
    lwd_choice = st.selectbox("Loading point water type", list(LOADING_WATER_PRESETS.keys()),
                               key="lwd_preset")
    preset_lwd = LOADING_WATER_PRESETS[lwd_choice]
    if preset_lwd is not None:
        lwd_default = preset_lwd
        lwd_badge_color = "#0369a1" if preset_lwd >= 1.02 else "#065f46"
        st.markdown(
            f"<span class='badge badge-blue'>Preset: {preset_lwd:.4f} t/m³ — {lwd_choice}</span>",
            unsafe_allow_html=True)
    else:
        lwd_default = 1.000

    rho_loading = st.number_input(
        "Loading water density (t/m³)",
        min_value=0.990, max_value=1.030,
        value=float(lwd_default), step=0.001, format="%.4f",
        help=(
            "Density of the water AT THE LOADING POINT. This directly determines displacement "
            "and hence draft while loading. River locations (San Barth, Awoba, "
            "Chapel/JasmineS bar) are effectively freshwater (1.000 t/m³). "
            "Offshore BIA berths use Nigerian coastal seawater (1.0255 t/m³). "
            "Using 1.0255 for river loading overestimates cargo by ~3,000+ Bbls."
        ),
    )
    st.markdown(
        f"<div style='font-size:0.68rem;color:var(--muted);font-family:var(--mono);margin-top:2px'>"
        f"Cb derived at ρ_ref=1.025 (IACS stability standard)"
...
<div style="display:flex;gap:8px;margin-bottom:1rem;flex-wrap:wrap">
  ...
  <span class="badge badge-lock">ρ_sea {SEAWATER_DENSITY_NG} t/m³ 🔒</span>
  <span class="badge badge-blue">ρ_load {rho_loading:.4f} t/m³</span>
</div>""", unsafe_allow_html=True)
...
with tab1:
    ...
    info(
        ...
        "<strong>Cb method</strong> (IACS / workbook standard). "
        f"Loading water ρ: <strong>{rho_loading:.4f} t/m³</strong> (set in sidebar). "
        "Both limits enforced: volume ≤ 98% tank capacity AND mass ≤ Tropical DWT."
    )
    ...
    with col_res:
        res = volume_to_draft(vessel, cargo_m3, api, bunker_mt, fw_mt, constant_mt, rho_loading)
        ...
        summary_df([
            ...
            ("Loading Water ρ",      f"{rho_loading:.4f} t/m³  (loading point)"),
            ("Offshore Seawater ρ",  f"{SEAWATER_DENSITY_NG} t/m³  🔒 locked"),
...
with tab2:
    ...
    info(
        ...
        "The calculator back-calculates achievable cargo volume using the "
        "<strong>Cb method</strong> (IACS / workbook standard). "
        f"Loading water \u03c1: <strong>{rho_loading:.4f} t/m\u00b3</strong> (sidebar). "
        "Both limits enforced: volume \u2264 98% tank, mass \u2264 Tropical DWT."
    )
    ...
    with c_d2:
        res2 = draft_to_volume(vessel, draft_inp, api, bunker_mt, fw_mt, constant_mt, rho_loading)
...
with tab3:
    ...
    for nm, v in sorted(db_now.items()):
        ...
        try:
            ...
            r   = volume_to_draft(v, lv["max_cargo_m3"], api, bunker_mt, fw_mt, constant_mt, rho_loading)
